...
public class SimpleConfig extends YamlConfiguration {
	...
	/**
	 * Optionally, you can set the header that will appear when the file gets edited
	 * automatically.
	 *
	...
	/**
	 * Path prefix is what appears automatically before the path, when getting keys
	 * from the file.
	 *
	 * Example: if you have a PlayerCache class that will always refer to a certain
	 * player, you can set his unique id as the pathPrefix to save you time and
	 * energy while getting his values, so instead of typing getString(id + ".key")
	 * you only need to type getString("key")
	 *
	 * Only works when the default file does not exist! (logically, in the example
	 * above you cannot create default values for each player out there :))
	 */
	 ...
	 *
	 * NB: Make sure you created the file with the exact same name and all the
	 * default values inside of your plugin in the src/main/resources folder!
	 *
	 ...
	 *
	 * @param fileName
	 * @param          useDefaults, require the default file? see commentaries to
	...
	public SimpleConfig(String fileName, boolean useDefaults) {
		...
		if (useDefaults) {
			// Now we use the file in your plugin .jar as defaults for updating the file on
			// the disk.
			this.defaults = YamlConfiguration.loadConfiguration(new InputStreamReader(
					SimpleConfig.class.getResourceAsStream("/" + fileName), StandardCharsets.UTF_8));
	...
	/**
	 * Writes a key with a value to your file. Example: write("weather.disable",
	 * true)
	 *
	 * @param path, the path, use '.' to split sections
	 * @param value, the value, can be a primitive, a String, HashMap or a
	 *        Collection (List, or a Set)
	 */
	...
	/**
	 * Gets an unspecified value from your file, so you must cast it to your desired
	 * value (example: (boolean) get("disable.this.feature", true)) The "def" is the
	 * default value, must be null since we use default values from your file in
	 * your .jar.
	 */
	@Override
	public Object get(String path, Object def) {
		...
		// hacky workaround: prevent infinite loop due to how get works in the parent
		// class
	...
	private File extract(String path) {
		final JavaPlugin i = Core.getInstance();
	...
	// Creates YourPlugin folder in plugins/ and the necessary file (e.g.
	// settings.yml)
	private File createFileAndDirectory(String path) {

		// The data folder is your plugin's folder with your plugin's name inside
		// plugins/ folder.
