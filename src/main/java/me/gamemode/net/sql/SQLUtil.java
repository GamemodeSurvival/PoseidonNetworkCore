...
public class SQLUtil {
	...
	private static String query(String table, String variable, Where where) {
		return "SELECT " + getChar(variable) + variable + getChar(variable) + " FROM `" + table + "` WHERE "
				+ where.getWhere();
	}
	...
	public static boolean execute(SQLConnection connection, String table, Map<String, Object> preset)
			throws ClassNotFoundException, SQLException {
		return connection.executeSQL(execute(table, preset));
	}

	public static int update(SQLConnection connection, String table, String variable, Object to, Where where)
			throws ClassNotFoundException, SQLException {
		return connection.updateSQL(update(table, variable, to, where));
	}

	public static ResultSet query(SQLConnection connection, String table, String variable, Where where)
			throws ClassNotFoundException, SQLException {
		return connection.querySQL(query(table, variable, where));
	}

	public static boolean createTable(SQLConnection sql, String table,
			Map<String, Map.Entry<SQLDataType, Integer>> data) throws ClassNotFoundException, SQLException {
		return sql.executeSQL(createTable(table, data));
	}

	public static boolean tableExists(SQLConnection connection, String table)
			throws ClassNotFoundException, SQLException {
